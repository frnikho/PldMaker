...
export class CalendarPage extends React.Component<CalendarPageProps, CalendarPageState> {
  ...
  private showState(authContext: UserContextProps) {
    if (authContext.isLogged === LoginState.logged) {
      ...
      return <CalendarComponent loadCalendar={() => this.load(authContext)} calendarId={this.props.params['calendarId'] ?? ''} orgId={this.props.params['id'] ?? ''} calendar={this.state.calendar} org={this.state.org}/>
